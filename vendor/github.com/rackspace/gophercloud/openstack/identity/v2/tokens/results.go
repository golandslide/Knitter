...
func (result CreateResult) ExtractToken() (*Token, error) {
	...
	klog.Infof("token-id: [%v]", response.Access.Token.ID)
	klog.Infof("issued_at before normalize: [%v]", response.Access.Token.IssuedAt)
	issuedAt := gophercloud.NormalizeTimeStr(response.Access.Token.IssuedAt)
	klog.Infof("issued_at after normalize: [%v]", issuedAt)
	issuedTs, err := time.Parse(gophercloud.RFC3339Milli, issuedAt)
	if err != nil {
		klog.Errorf("parse issued_at error: [%v]", err.Error())
	}
