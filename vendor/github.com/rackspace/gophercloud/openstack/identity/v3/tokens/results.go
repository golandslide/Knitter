...
func (r commonResult) ExtractToken() (*Token, error) {
	...
	klog.Infof("issued_at before normalize: [%v]", response.Token.IssuedAt)
	issuedAt := gophercloud.NormalizeTimeStr(response.Token.IssuedAt)
	klog.Infof("issued_at after normalize: [%v]", issuedAt)
	token.IssuedAt, err = time.Parse(gophercloud.RFC3339Milli, issuedAt)
	if err != nil {
