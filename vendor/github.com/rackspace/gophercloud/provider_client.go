...
const (
	DefaultUserAgent = "gophercloud/1.0.0"
	ReAuthLeastLeftTime time.Duration = 3 * time.Minute
	LeastValidTokenInterval time.Duration = 10 * time.Minute
)
...
func (client *ProviderClient) isTokensUrl(url string) bool {
	u, _ := NETURL.Parse(url)
	if u != nil && strings.Contains(u.Path, "tokens") {
		return true
	}
...
func (client *ProviderClient) reauthTimeReached() bool {
	remoteTimeInterval := client.TokenExpiresAt.Sub(client.TokenIssuedAt)
	if remoteTimeInterval <= LeastValidTokenInterval {
		klog.Warningf("token valid time[%v] is less than [%v]", remoteTimeInterval.String(), LeastValidTokenInterval.String())
		return false
	}

	localTimeInterval := time.Since(client.TokenIssuedAtLocal)
	return (remoteTimeInterval - localTimeInterval) <= ReAuthLeastLeftTime
}

// Request performs an HTTP request using the ProviderClient's current HTTPClient. An authentication
// header will automatically be provided.
func (client *ProviderClient) Request(method, url string, options RequestOpts) (*http.Response, error) {
	if client.ReauthFunc != nil && !client.isTokensUrl(url) && client.reauthTimeReached() {
		klog.Infof("ReAuth Trigger")
		err := client.ReauthFunc()
		if err != nil {
			klog.Warningf("ReAuth failed, error: [%v]", err.Error())
		} else {
			klog.Infof("ReAuth OK")
		}
	}
	...
	if options.JSONBody != nil {
		...
		klog.Infof("REQ-BODY: [%v]", string(rendered))
	...
	if err != nil {
		klog.Infof("ERR: [%v]", err.Error())
		return nil, err
	}
	klog.Infof("RSP: [%v]", *resp)
	...
	if !ok {
		...
		klog.Infof("RSP-BODY: [%v]", string(body))
		resp.Body.Close()
